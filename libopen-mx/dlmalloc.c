/*
 * Open-MX specific modifications
 */
...
/* directly use syscall for mremap/munmap so that they cannot be intercepted */
#include <unistd.h>
#include <sys/syscall.h>

static void * dlmremap(void * old_ptr, size_t old_size, size_t new_size, int maymove)
{
  return (void *) syscall(__NR_mremap, old_ptr, old_size, new_size, maymove);
}

static int dlmunmap(void *old_ptr, size_t old_size)
{
  return (int) syscall(__NR_munmap, old_ptr, old_size);
}

/*
 * End of Open-MX specific modifications
 */
...
#ifndef WIN32
#define MUNMAP_DEFAULT(a, s)  dlmunmap((a), (s))
...
static void* internal_memalign(mstate m, size_t alignment, size_t bytes) {
  ...
  if (bytes >= MAX_REQUEST - alignment) {
    ...
    char* mem = (char*)(unsigned long)internal_malloc(m, req);
    if (mem != 0) {
      void* leader = 0;
      void* trailer = 0;
      mchunkptr p = mem2chunk(mem);
...
static void** ialloc(mstate m,
  ...
  */
  ...
  if (chunks != 0) {
    ...
    if (n_elements == 0)
      return (void**)(unsigned long)internal_malloc(m, 0);
    marray = 0;
