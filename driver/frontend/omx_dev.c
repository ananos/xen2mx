...
static int omx_endpoint_alloc_resources(struct omx_endpoint *endpoint)
{
	...
	if (!userdesc) {
		printk(KERN_ERR
		       "Open-MX: failed to allocate endpoint user descriptor\n");
		ret = -ENOMEM;
		goto out;
	}
	userdesc->status = 0;
	...
	sendq_pages =
	    kmalloc(OMX_SENDQ_SIZE / PAGE_SIZE * sizeof(struct page *),
		    GFP_KERNEL);
	if (!sendq_pages) {
		printk(KERN_ERR
		       "Open-MX: failed to allocate sendq pages array\n");
	...
	recvq_pages =
	    kmalloc(OMX_RECVQ_SIZE / PAGE_SIZE * sizeof(struct page *),
		    GFP_KERNEL);
	if (!recvq_pages) {
		printk(KERN_ERR
		       "Open-MX: failed to allocate recvq pages array\n");
 ...
 */
static void omx_endpoint_destroy_workfunc(omx_work_struct_data_t data)
{
	struct omx_endpoint *endpoint =
	    OMX_WORK_STRUCT_DATA(data, struct omx_endpoint, destroy_work);
	omx_endpoint_free_resources(endpoint);
	kfree(endpoint);
}

/* Called when the last reference on the endpoint is released */
void __omx_endpoint_last_release(struct kref *kref)
{
	struct omx_endpoint *endpoint =
...
omx_endpoint_open(struct omx_endpoint *endpoint, const void __user * uparam)
{
	...
	if (!(dev_get_flags(ifp) & IFF_UP))
		endpoint->userdesc->status |=
		    OMX_ENDPOINT_DESC_STATUS_IFACE_DOWN;
	if (ifp->mtu < OMX_MTU)
		endpoint->userdesc->status |=
		    OMX_ENDPOINT_DESC_STATUS_IFACE_BAD_MTU;
	if (!omx_iface_get_rx_coalesce(ifp, &rx_coalesce)
	    && rx_coalesce >= OMX_IFACE_RX_USECS_WARN_MIN)
		endpoint->userdesc->status |=
		    OMX_ENDPOINT_DESC_STATUS_IFACE_HIGH_INTRCOAL;
 ...
 */
int omx_endpoint_close(struct omx_endpoint *endpoint, int ifacelocked)
{
	int ret;
	...
	if (endpoint->xen)
		omx_ioctl_xen_close_endpoint(endpoint, NULL);
...
struct omx_endpoint *omx_endpoint_acquire_by_iface_index(const struct omx_iface
							 *iface, uint8_t index)
{
	struct omx_endpoint *endpoint;
	int err;
...
static int omx_miscdev_open(struct inode *inode, struct file *file)
{
	struct omx_endpoint *endpoint;
	int ret = 0;
	dprintk_in();
	...
	endpoint->fe = __omx_xen_frontend;
	endpoint->xen = 0;
out:
	dprintk_out();
	return ret;
...
static int omx_miscdev_release(struct inode *inode, struct file *file)
{
...
static int (*omx_ioctl_with_endpoint_handlers[])(struct omx_endpoint * endpoint, void __user * uparam) = {
       ...
       [OMX_EPCMD_XEN_OPEN_ENDPOINT]           = omx_ioctl_xen_open_endpoint,
       [OMX_EPCMD_XEN_CLOSE_ENDPOINT]          = omx_ioctl_xen_close_endpoint,
       [OMX_EPCMD_XEN_CREATE_USER_REGION]      = omx_ioctl_xen_user_region_create,
       [OMX_EPCMD_XEN_DESTROY_USER_REGION]     = omx_ioctl_xen_user_region_destroy,
       [OMX_EPCMD_XEN_GET_BOARD_INFO]          = omx_ioctl_xen_get_board_info,
       [OMX_EPCMD_XEN_SEND_NOTIFY]             = omx_ioctl_xen_send_notify,
       [OMX_EPCMD_XEN_SEND_CONNECT_REQUEST]    = omx_ioctl_xen_send_connect_request,
       [OMX_EPCMD_XEN_SEND_CONNECT_REPLY]      = omx_ioctl_xen_send_connect_reply,
       [OMX_EPCMD_XEN_SEND_LIBACK]             = omx_ioctl_xen_send_liback,
       [OMX_EPCMD_XEN_SEND_RNDV]               = omx_ioctl_xen_send_rndv,
       [OMX_EPCMD_XEN_SEND_TINY]               = omx_ioctl_xen_send_tiny,
       [OMX_EPCMD_XEN_SEND_SMALL]              = omx_ioctl_xen_send_small,
       [OMX_EPCMD_XEN_PULL]                    = omx_ioctl_xen_pull,
       [OMX_EPCMD_XEN_SEND_MEDIUMVA]           = omx_ioctl_xen_send_mediumva,
       [OMX_EPCMD_XEN_SEND_MEDIUMSQ_FRAG]           = omx_ioctl_xen_send_mediumsq_frag,
};
...
omx_miscdev_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
	...
	dprintk_deb("cmd_index = %#x, cmd=%#x\n", cmd_index, cmd);
	if (likely
	    (handler_offset < ARRAY_SIZE(omx_ioctl_with_endpoint_handlers))) {
		struct omx_endpoint *endpoint = file->private_data;

		dprintk_deb("handler_offset = %d\n", handler_offset);
		/*
		...
		dprintk_deb("will call the relevant handler\n");
		ret =
		    omx_ioctl_with_endpoint_handlers[(unsigned char)
						     handler_offset] (endpoint,
								      (void
								       __user *)
								      arg);
		dprintk_deb("didn't make it through here...\n");
		goto out;
	}

	switch (cmd) {
	case OMX_CMD_XEN_PEER_FROM_INDEX:
	case OMX_CMD_XEN_PEER_FROM_ADDR:
	case OMX_CMD_XEN_PEER_FROM_HOSTNAME:{
			struct omx_cmd_misc_peer_info peer_info;
			unsigned local_cmd = 0x666;
			...
			if (unlikely(ret != 0)) {
				ret = -EFAULT;
				printk(KERN_ERR
				       "Open-MX: Failed to read '%s' command argument, error %d\n",
				       omx_strcmd(cmd), ret);
				goto out;
			}
			...
			if (cmd == OMX_CMD_XEN_PEER_FROM_INDEX)
				local_cmd = OMX_CMD_PEER_FROM_INDEX;
			else if (cmd == OMX_CMD_XEN_PEER_FROM_ADDR)
				local_cmd = OMX_CMD_PEER_FROM_ADDR;
			else if (cmd == OMX_CMD_XEN_PEER_FROM_HOSTNAME)
				local_cmd = OMX_CMD_PEER_FROM_HOSTNAME;

			ret =
			    omx_xen_peer_lookup(&peer_info.index,
						&peer_info.board_addr,
						peer_info.hostname, local_cmd);
			if (ret < 0)
				break;
			...
			if (unlikely(ret != 0)) {
				ret = -EFAULT;
				printk(KERN_ERR
				       "Open-MX: Failed to write '%s' command result, error %d\n",
				       omx_strcmd(cmd), ret);
			}
			break;
		}

	case OMX_CMD_XEN_GET_ENDPOINT_INFO:{
			struct omx_cmd_get_endpoint_info get_endpoint_info;

			ret =
			    copy_from_user(&get_endpoint_info,
					   (void __user *)arg,
					   sizeof(get_endpoint_info));
			if (unlikely(ret != 0)) {
				ret = -EFAULT;
				printk(KERN_ERR
				       "Open-MX: Failed to read get_endpoint_info command argument, error %d\n",
				       ret);
				goto out;
			}

			dprintk_deb("asking info for bidx = %#lx, eidx = %#lx\n",
				    (unsigned long) get_endpoint_info.board_index,
				    (unsigned long) get_endpoint_info.endpoint_index);
			ret =
			    omx_xen_endpoint_get_info(get_endpoint_info.
						      board_index,
						      get_endpoint_info.
						      endpoint_index,
						      &get_endpoint_info.info);
			ret =
			    copy_to_user((void __user *)arg, &get_endpoint_info,
					 sizeof(get_endpoint_info));
			if (unlikely(ret != 0)) {
				ret = -EFAULT;
				printk(KERN_ERR
				       "Open-MX: Failed to write get_endpoint_info command result, error %d\n",
				       ret);
			}
			break;
		}
	case OMX_CMD_GET_BOARD_COUNT:{
			...
			if (unlikely(ret != 0)) {
				ret = -EFAULT;
				printk(KERN_ERR
				       "Open-MX: Failed to write get_board_count command result, error %d\n",
				       ret);
			}
			break;
		}
	case OMX_CMD_XEN_GET_BOARD_COUNT:{
			uint32_t count;
			omx_xen_ifaces_get_count(&count);
	...
	case OMX_CMD_XEN_PEER_TABLE_GET_STATE:{
			...
			if (unlikely(ret != 0)) {
				ret = -EFAULT;
				printk_err(
				       "Failed to write get peer table state command result, error %d\n",
				       ret);
			}
			break;
		}
	...
	case OMX_CMD_GET_BOARD_INFO:{
			...
			if (endpoint->status != OMX_ENDPOINT_STATUS_OK) {
				...
				get_board_info.board_index =
				    endpoint->board_index;
			}

			ret =
			    omx_iface_get_info(get_board_info.board_index,
					       &get_board_info.info);
			if (ret < 0)
				goto out;
	...
	case OMX_CMD_GET_ENDPOINT_INFO:{
			...
			ret =
			    omx_endpoint_get_info(get_endpoint_info.board_index,
						  get_endpoint_info.
						  endpoint_index,
						  &get_endpoint_info.info);
			ret =
	...
	case OMX_CMD_GET_COUNTERS:{
			...
			if (unlikely(ret != 0)) {
				ret = -EFAULT;
				printk(KERN_ERR
				       "Open-MX: Failed to read get_counters command argument, error %d\n",
				       ret);
				goto out;
			}
			...
			if (unlikely(ret != 0)) {
				ret = -EFAULT;
				printk(KERN_ERR
				       "Open-MX: Failed to write get_counters command result, error %d\n",
				       ret);
			}
			break;
		}
	...
	case OMX_CMD_PEER_TABLE_GET_STATE:{
			...
			if (unlikely(ret != 0)) {
				ret = -EFAULT;
				printk(KERN_ERR
				       "Open-MX: Failed to write get peer table state command result, error %d\n",
				       ret);
			}
			break;
		}
	...
	case OMX_CMD_PEER_ADD:{
			...
			if (unlikely(ret != 0)) {
				ret = -EFAULT;
				printk(KERN_ERR
				       "Open-MX: Failed to read add_peer command argument, error %d\n",
				       ret);
				goto out;
			}
			...
			if (peer_info.hostname[0] == '\0') {
				...
				peer_info.hostname[OMX_HOSTNAMELEN_MAX - 1] =
				    '\0';
	...
	case OMX_CMD_PEER_FROM_HOSTNAME:{
			...
			if (cmd == OMX_CMD_PEER_FROM_INDEX)
				...
				ret =
				    omx_peer_lookup_by_addr(peer_info.
							    board_addr,
							    peer_info.hostname,
							    &peer_info.index);
			else if (cmd == OMX_CMD_PEER_FROM_HOSTNAME)
				ret =
				    omx_peer_lookup_by_hostname(peer_info.
								hostname,
								&peer_info.
								board_addr,
								&peer_info.
...
out:
	if (ret != 0) {
		...
		dprintk_deb("cmd %x (%x,%s) returns %d\n",
			cmd, cmd_index, omx_strcmd(cmd), ret);
	}
...
static int omx_miscdev_mmap(struct file *file, struct vm_area_struct *vma)
{
	struct omx_endpoint *endpoint = file->private_data;
	...
	dprintk_in();
	/* endpoint-less ioctl */
	if (offset == OMX_DRIVER_DESC_FILE_OFFSET
	    && size == PAGE_ALIGN(OMX_DRIVER_DESC_SIZE)) {
		if (vma->vm_flags & (VM_WRITE | VM_MAYWRITE)) {	/* cannot mmap for writing and should not even open for writing */
			ret = -EPERM;
			goto out;
		}
		ret = omx_remap_vmalloc_range(vma, omx_driver_userdesc, 0);
		goto out;
	}
	...
	if (endpoint->status != OMX_ENDPOINT_STATUS_OK) {
		printk(KERN_INFO
		       "Open-MX: Cannot map endpoint resources from a closed endpoint, status=%d\n", endpoint->status);
		ret = -EINVAL;
		goto out;
	}

	if (offset == OMX_ENDPOINT_DESC_FILE_OFFSET
	    && size == PAGE_ALIGN(OMX_ENDPOINT_DESC_SIZE)) {
		ret = omx_remap_vmalloc_range(vma, endpoint->userdesc, 0);
		goto out;
		...
		ret = omx_remap_vmalloc_range(vma, endpoint->sendq, 0);
		goto out;
		...
		ret = omx_remap_vmalloc_range(vma, endpoint->recvq, 0);
		goto out;
		...
		ret = omx_remap_vmalloc_range(vma, endpoint->exp_eventq, 0);
		goto out;
		...
		ret = omx_remap_vmalloc_range(vma, endpoint->unexp_eventq, 0);
		goto out;
...
ssize_t
    static omx_miscdev_read(struct file *filp, char __user * buff, size_t count,
			    loff_t * offp)
{
	ssize_t ret = 0;
	char *buffer;
...
static struct file_operations omx_miscdev_fops = {
	.owner = THIS_MODULE,
...
static struct miscdevice omx_miscdev = {
...
int omx_dev_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(omx_ioctl_with_endpoint_handlers); i++)
		if (omx_ioctl_with_endpoint_handlers[i] == NULL) {
			printk(KERN_ERR
			       "Open-MX: Found a hole in the array of endpoint-based ioctl handlers at offset %d\n",
			       i);
			return -EINVAL;
		}
#endif
	...
	if (OMX_SENDQ_SIZE & ~PAGE_MASK) {
		printk(KERN_ERR
		       "Open-MX: Cannot use sendq with non-page-aligned size %lx\n",
		       OMX_SENDQ_SIZE);
		return -EINVAL;
	}
	if (OMX_RECVQ_SIZE & ~PAGE_MASK) {
		printk(KERN_ERR
		       "Open-MX: Cannot use recvq with non-page-aligned size %lx\n",
		       OMX_RECVQ_SIZE);
		return -EINVAL;
	}
	if (OMX_EXP_EVENTQ_SIZE & ~PAGE_MASK) {
		printk(KERN_ERR
		       "Open-MX: Cannot use exp eventq with non-page-aligned size %lx\n",
		       OMX_EXP_EVENTQ_SIZE);
		return -EINVAL;
	}
	if (OMX_UNEXP_EVENTQ_SIZE & ~PAGE_MASK) {
		printk(KERN_ERR
		       "Open-MX: Cannot use unexp eventq with non-page-aligned size %lx\n",
	...
	if (ret < 0) {
		printk(KERN_ERR
		       "Open-MX: Failed to register misc device, error %d\n",
		       ret);
		goto out;
	}
...
void omx_dev_exit(void)
{
	misc_deregister(&omx_miscdev);
}

/* Helper functions for Xen-OMX */
