...
omx_iface_get_info(uint32_t board_index, struct omx_board_info *info)
{
	...
	if (board_index == OMX_SHARED_FAKE_IFACE_INDEX) {
		...
		ret = -EINVAL;
		if (board_index >= omx_iface_max) {
			printk_err("board index = %d, omx_iface_max = %d\n", board_index, omx_iface_max);
			goto out_with_lock;
		}
		dprintk_deb("%s: board =%lu\n", __func__, board_index);

		iface = rcu_dereference(omx_ifaces[board_index]);
		dprintk_deb("%s: iface=%#lx\n", __func__, iface);
		if (!iface) {
			printk_err("iface = %#lx\n", (unsigned long) iface);
			goto out_with_lock;
		}
