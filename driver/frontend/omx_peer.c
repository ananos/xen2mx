...
omx_peer_add(uint64_t board_addr, const char *hostname)
{
	...
	dprintk_deb("%s: iface = %#lx, hostname = %s\n", __func__, (unsigned long) iface, new_hostname);
	omx_ifaces_peers_unlock();
...
omx_local_peer_acquire_endpoint(uint16_t peer_index, uint8_t endpoint_index)
{
	struct omx_peer *peer;
	struct omx_iface *iface;
	struct omx_endpoint *endpoint, *err = NULL;
			 ...
			 uint64_t *board_addr, char *hostname)
{
	...
	if (hostname) {
		if (peer->hostname)
			...
			hostname[0] = '\0';
		dprintk_deb("%s: hostname = %s, %s\n", __func__, peer->hostname, hostname);
	}
