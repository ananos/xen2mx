			     ...
			     void __user * uparam)
{
	...
	spin_lock_init(&region->status_lock);
...
omx_user_region_acquire(const struct omx_endpoint * endpoint,
			uint32_t rdma_id)
{
	struct omx_user_region * region, *ret = NULL;
			      ...
			      unsigned long length)
{
	...
	if (omx_dmaengine && length >= omx_dma_sync_min) {
		ret = omx_dma_copy_between_user_regions(src_region, src_offset, dst_region, dst_offset, length);
		goto out;
	}
	else {
#endif /* OMX_HAVE_DMA_ENGINE */
		ret = omx_memcpy_between_user_regions_to_current(src_region, src_offset, dst_region, dst_offset, length);
		goto out;
	}
out:
	dprintk_out();
