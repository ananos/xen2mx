...
omx_endpoint_acquire_by_iface_index(const struct omx_iface * iface, uint8_t index)
{
	...
	dprintk_deb("in lock, endpoint index =%u\n", index);
	...
	dprintk_deb("in lock, endpoint @%#lx\n", endpoint);
	...
	dprintk_deb("in lock, increasing refcount%u\n", atomic_read(&endpoint->refcount.refcount));
	...
	dprintk_deb("in lock, status is %#x\n", endpoint->status);
	rcu_read_unlock();
	dprintk_out();
	return endpoint;
