...
timers_t t_send_tiny, t_send_small, t_send_medium, t_send_connect, t_send_notify, t_send_connect_reply, t_send_rndv, t_send_liback;
...
omx_medium_frag_skb_destructor(struct sk_buff *skb)
{
	...
	if (endpoint->xen) {
			...
			dprintk(PULL, "XEN ENDPOINT! Linear MEDIUMSQ DONE!@%#lx\n", (unsigned long) omx_xenif);
		    ...
		    void __user * uparam)
{
	...
	struct omx_cmd_send_tiny_hdr *cmd_xen = NULL;
	struct omx_iface * iface = endpoint->iface;
	struct net_device * ifp = iface->eth_ifp;
	...
	if (endpoint->xen) {
		cmd_xen = &((struct omx_cmd_send_tiny __user *) uparam)->hdr;
		if (!cmd_xen) {
			printk_err("cmd is NULL!!!\n");
			ret = -EINVAL;
			goto out;
		}
		length = cmd_xen->length;
	} else {
	...
	if (endpoint->xen) {
		if (unlikely(cmd_xen->shared)) {
			ret = omx_shared_send_tiny(endpoint, cmd_xen, &((struct omx_cmd_send_tiny __user *) uparam)->data);
			goto out;
		}
	} else {
		if (unlikely(cmd.shared)) {
			ret = omx_shared_send_tiny(endpoint, &cmd, &((struct omx_cmd_send_tiny __user *) uparam)->data);
			goto out;
		}
	}
	...
	/* set destination peer */
	if (endpoint->xen)
		ret = omx_set_target_peer(ph, iface, cmd_xen->peer_index);
	else
		ret = omx_set_target_peer(ph, iface, cmd.peer_index);
	if (ret < 0) {
	...
	if (endpoint->xen) {
		OMX_HTON_8(tiny_n->dst_endpoint, cmd_xen->dest_endpoint);
		OMX_HTON_16(tiny_n->lib_seqnum, cmd_xen->seqnum);
		OMX_HTON_16(tiny_n->lib_piggyack, cmd_xen->piggyack);
		OMX_HTON_32(tiny_n->session, cmd_xen->session_id);
		OMX_HTON_16(tiny_n->checksum, cmd_xen->checksum);
		OMX_HTON_MATCH_INFO(tiny_n, cmd_xen->match_info);
	} else {
		     ...
		     void __user * uparam)
{
	...
	if (unlikely(cmd.shared)) {
		ret = omx_shared_send_small(endpoint, &cmd);
		goto out;
	}
			     ...
			     void __user * uparam)
{
	...
	if (unlikely(cmd.shared)) {
		ret = omx_shared_send_mediumsq_frag(endpoint, &cmd);
		goto out;
	}
	...
	if (unlikely(frag_length > omx_skb_copy_max
		...
		while (remaining) {
			...
			if (chunk > PAGE_SIZE)
				chunk = PAGE_SIZE;
			if (endpoint->xen) {
				page = endpoint->xen_sendq_pages[current_sendq_offset >> PAGE_SHIFT];
			} else {
		...
		if (endpoint->xen) {
			/* attach the sendq page */
			uint32_t current_sendq_offset = sendq_offset;
			uint32_t remaining = frag_length;
			while (remaining) {
				unsigned int chunk = remaining;
				if (chunk > PAGE_SIZE)
					chunk = PAGE_SIZE;
				page = endpoint->xen_sendq_pages[current_sendq_offset >> PAGE_SHIFT];
				get_page(page);
				memcpy(data, pfn_to_kaddr(page_to_pfn(page)) + (current_sendq_offset & (~PAGE_MASK)), chunk);
				remaining -= chunk;
				current_sendq_offset += chunk;
			}
		} else {
			...
			void __user * uparam)
{
	...
	void __user * cur_udata = NULL;
	void *cur_udata_xen = NULL;
	uint32_t nseg;
	int ret;
	int frags_nr;
     	int i;
	...
	/* FIXME: probably the same thing ;-), psomas looked into it,
	 * the __user attribute only refers to a static analysis helper
	 * and it's simply ignored */
	if (!endpoint->xen) {
		ret = copy_from_user(&cmd, uparam, sizeof(cmd));
		if (unlikely(ret != 0)) {
	...
	if (unlikely(cmd.shared)) {
		ret = omx_shared_send_mediumva(endpoint, &cmd);
		goto out;
	}

	/* FIXME: Really hacky stuff!
	 * instead of re-constructing the virtual area we got from the frontend via the page_list
	 * the frontend granted to us, we just assign each page to a segment.
	 * This seems a more clean way to send stuff as close to Open-MX, but its mainly
	 * inefficient and could lead to some kind of corruption (the one we're seeing maybe :S)
	 */
	if (!endpoint->xen) {
		/* get user segments */
		...
		usegs = (void *)(unsigned long) cmd.segments;
	/* compute the segments length */
	remaining = 0;
	for(i=0; i<nseg; i++) {
		remaining += usegs[i].len;
		dprintk_deb("remaining= %#x, usegs[%d]=%#lx, usegs[%d]= %#lx\n", remaining, i, usegs[i].vaddr, i, usegs[i].len);
	}
	...
	if (endpoint->xen) {
		cur_udata_xen = (void*) (unsigned long) cur_useg->vaddr;
	}
	else {
	...
	for(i=0; i<frags_nr; i++) {
		...
		/* FIXME: are we sure everything works as expected here ? */
		/* copy the data right after the header */
		while (frag_remaining) {
			uint16_t chunk = frag_remaining > cur_useg_remaining ? cur_useg_remaining : frag_remaining;

			if (!endpoint->xen) {
				dprintk_deb("data =%p, cur_udata=%p, chunk=%#x\n", data, cur_udata, chunk);
				ret = copy_from_user(data, cur_udata, chunk);
			}
			else
			{
				if (memcpy(data, cur_udata_xen, chunk) == NULL) {
					ret = -EFAULT;
				}

				dprintk_deb("data =%p, cur_udata_xen=%p, chunk=%#x\n", data, cur_udata_xen, chunk);
			}
			if (unlikely(ret != 0)) {
					printk(KERN_ERR "Open-MX: Failed to read send mediumva cmd data, ret = %d\n", ret);
					ret = -EFAULT;
					goto out_with_skb;
				}
		    ...
		    void __user * uparam)
{
	...
	if (unlikely(cmd.shared)) {
		printk_err("Shouldn't cross this path shared\n");
		ret = omx_shared_send_rndv(endpoint, &cmd);
		goto out;
	}
	...
	if (!omx_pin_synchronous) {
		...
		printk_err("Shouldn't cross this path (pin_synchronous)\n");
		      ...
		      void __user * uparam)
{
	...
	if (unlikely(cmd.shared)) {
		ret = omx_shared_send_notify(endpoint, &cmd);
		goto out;
	}
		      ...
		      void __user * uparam)
{
	...
	if (unlikely(cmd.shared)) {
		ret = omx_shared_send_liback(endpoint, &cmd);
		goto out;
	}
