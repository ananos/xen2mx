...
omx__get_board_count(uint32_t * count)
{
  ...
  err = ioctl(omx__globals.control_fd, OMX_CMD_XEN_GET_BOARD_COUNT, count);
  if (err < 0)
...
omx__get_board_info(const struct omx_endpoint * ep, uint32_t index, struct omx_board_info * info)
{
  ...
  err = ioctl(fd, OMX_CMD_XEN_GET_BOARD_INFO, &get_info);
  if (err < 0) {
    if (!ep) {
...
omx__get_board_index_by_name(const char * name, uint32_t * index)
{
  ...
  for(i=0; i<max; i++) {
    ...
    err = ioctl(omx__globals.control_fd, OMX_CMD_XEN_GET_BOARD_INFO, &board_info);
    if (err < 0) {
      omx__ioctl_errno_to_return_checked(OMX_INTERNAL_MISC_EINVAL,
					 OMX_SUCCESS,
