				 ...
				 uint32_t *size, uint64_t *mapper_id)
{
  ...
  err = ioctl(omx__globals.control_fd, OMX_CMD_XEN_PEER_TABLE_GET_STATE, &state);
  if (err < 0) {
...
omx__driver_peer_from_index(uint32_t index, uint64_t *board_addr, char *hostname)
{
  ...
  err = ioctl(omx__globals.control_fd, OMX_CMD_XEN_PEER_FROM_INDEX, &peer_info);
  if (err < 0) {
...
omx__driver_peer_from_addr(uint64_t board_addr, char *hostname, uint32_t *index)
{
  ...
  err = ioctl(omx__globals.control_fd, OMX_CMD_XEN_PEER_FROM_ADDR, &peer_info);
  if (err < 0) {
    omx__ioctl_errno_to_return_checked(OMX_INTERNAL_MISC_EINVAL,
				       OMX_SUCCESS,
				       "lookup peer by addr");
...
omx__driver_peer_from_hostname(const char *hostname, uint64_t *board_addr, uint32_t *index)
{
  ...
  err = ioctl(omx__globals.control_fd, OMX_CMD_XEN_PEER_FROM_HOSTNAME, &peer_info);
  if (err < 0) {
    omx__ioctl_errno_to_return_checked(OMX_INTERNAL_MISC_EINVAL,
				       OMX_SUCCESS,
